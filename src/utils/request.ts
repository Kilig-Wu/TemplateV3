...
const responseHandler = (response: AxiosResponse) => {
    ...
    if(data.code === 200 || data.success === true) {
        ...
        notification.error({ message: data.code, description: data.message, duration: 4 });
...
//get
export function useGet<R = any, T = any>(url: string, params?: T, config?: AxiosRequestConfig): Promise<R>{
    return request.get(url, { params, ...config })
}

export function usePost<R = any, T = any>(url: string, data?: T, config?: AxiosRequestConfig): Promise<R> {
    return request.post(url, data, config)
}

//put
export function usePut<R = any, T = any>(url: string, data?: T, config?: AxiosRequestConfig): Promise<R> {
    return axios.put(url, data, config);
}

//delete
export function useDelete<R = any, T = any>(url: string, params?: T, config?: AxiosRequestConfig): Promise<R> {
